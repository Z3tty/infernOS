...
static int usb_msd_reset(struct usb_msd_dev *msd)
{
	...
	struct usb_dev_setup_request reset_dev = {.bmRequestType = 0x21,
						  .bRequest = 0xff,
						  .wValue = 0,
						  .wIndex =
							  msd->udev->ifc_number,
...
static int usb_msd_read_write(struct usb_msd_dev *umd, int dir, int cb_size,
			      char *cb_data, int length, char *data)
{
	int cbw_size;
	int csw_size;
	int rc;
 ...
 */
static int usb_msd_read(void *driver, int cb_size, char *cb_data, int size,
			char *data)
{
...
static int usb_msd_write(void *driver, int cb_size, char *cb_data, int size,
			 char *data)
{
	struct usb_msd_dev *umd = (struct usb_msd_dev *)driver;
