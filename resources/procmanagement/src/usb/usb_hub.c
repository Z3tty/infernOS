...
void usb_hub_scan_ports()
{
	...
	for (port_i = 0; port_i < uhub_i->port_num; port_i++) {
		...
		if (previous_ps == USB_PORT_DISCONNECTED
		    && current_ps != USB_PORT_DISCONNECTED) {
			DEBUG("found a USB device, port status %d",
			      (int)current_ps);
			...
			/* Perform standard reset routine before device
			 * initialisation */
			DEBUG("resetting...");
			uhub_i->hub_ops->port_command(uhub_i, port_i,
			...
			udev->usb_sc =
				uhub_i->hub_ops->port_speed(uhub_i, port_i);
